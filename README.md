...
代理地址类：4种协议支持、代理地址定时更换、代理地址每次请求后更换、API接口自动获取代理地址、支持有身份鉴别的代理地址，支持代理地址有效性校验，支持负载均衡模式
...
# 代理地址轮换模式：cycle 表示循环使用，once 表示用完即止，load_balance 表示负载均衡(默认为:cycle)
mode = cycle
...
工具支持直接调用代理地址获取的API接口，当您配置`use_getip = True`时，工具将不再从本地ip.txt中读取代理地址，而是更换为通过执行 **getip.py** 脚本来获取新的代理地址（请记得给自己IP加白名单）。
...
- [x] 增加负载均衡模式，同时使用大量代理地址发送，从而有效提高并发效率，减少单一服务器负载
...
- [ ] 增加日志做的非常细，所有连接ProxyCat的IP身份都记录，支持多用户
